                    ...
                    <table id="all-registered-users-table" class="table table-test table-striped table-bordered nowrap hover display" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr v-for="row in all_history.data" :key="row.index">
                          <td>{{row.index}}. </td>
                          <td>
                            <button v-if="row.withdrawable == 1  && row.pending_amt > 0" class="btn btn-primary btn-round" @click="withdrawFunds(row)">
                              Withdraw
                            </button>
                          </td>
        ...
        <div class="modal-dialog">
          <div class="modal-content" >
            <div class="modal-header text-center">
              <h3 class="modal-title">Withdraw Savings</h3>
            ...
            <div class="modal-body" id="modal-body">
              <p>Enter amount you want to withdraw. Note: Maximum Is <em class="text-primary" v-html="'₦' + addCommas(maximum_amt_withdrawable)"></em></p>

              <form style="margin-top: 40px;" id="make-savings-form" @submit.prevent="submitWithdrawFundsForm">
                  <text-input :max="maximum_amt_withdrawable" v-model="withdraw_funds_form.amount" :error="withdraw_funds_form.errors.amount" type="number" label="Amount" placeholder="" required/>

                  
                  <loading-button :loading="withdraw_funds_form.processing" class="btn btn-primary col-12" type="submit">Submit</loading-button>
                </form>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      withdraw_funds_form: this.$inertia.form({
        amount: null,
        id: null,
      }),
  ...
  watch: {
    filter_rows_form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('view_savings_history'), pickBy(this.filter_rows_form), { preserveState: true,preserveScroll: true })
      }, 150),
    },
  },
  mounted() {
  ...
  methods: {
    ...
    submitWithdrawFundsForm(){
      var self = this;
      
      var amount = Number(self.withdraw_funds_form.amount);

      var text = "";
      
      text = "You are about to make a withdrawal of <em class='text-primary'>₦" + addCommas(amount) + "</em> from your savings. Are you sure you want to proceed with this withdrawal?";
      ...
      swal({
        ...
        self.withdraw_funds_form.post(self.route('process_coop_savings_withdrawals'), {
          preserveScroll: true,
          onSuccess: (page) => {
            ...
            if(response.success){
              ...
              text = "You have successfully made a withdrawal of <em class='text-primary'>₦" + addCommas(amount) + "</em> from your savings account to your mgr account.";
            ...
            }else if(response.invalid_withdrawal){
              swal({
                title: 'Ooops',
                text: "This saving is not valid for withdrawal",
                type: 'error'
              })
            }else{
              swal({
    ...
    withdrawFunds(row){
      var self = this;
      self.maximum_amt_withdrawable = row.pending_amt;
      self.withdraw_funds_form.id = row.id;
