      ...
      <div class="">
        <div class="container-fluid">
          <div class="text-right">
            <inertia-link as="button" class="btn btn-primary" :href="route('funds_transfer')"><i style="font-size: 13px;" class="fas fa-dollar-sign"></i>&nbsp;&nbsp;Transfer Funds</inertia-link>
                  ...
                  <div class='row justify-content-center'>
                    ...
                    <form id="filter-rows-form" class="col-sm-12" style="margin-bottom: 35px;">  
                      <div class="form-row">
                        ...
                        <text-input v-model="filter_rows_form.date" :error="filter_rows_form.errors.date" type="date" label="Transaction Date" id="date" placeholder="" class="col-sm-4"/>
                    ...
                    <table id="all-registered-users-table" class="table table-test table-striped table-bordered nowrap hover display" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr v-for="row in all_history.data" :key="row.index">
                          <td>{{row.index}}. </td>
                          <!-- <td v-html="row.sender_full_name"></td> -->
                          <td v-if="row.user_id != row.sender"><inertia-link class="nav-link" :href="route('user_profile', row.sender_slug)">{{row.sender_full_name}}</inertia-link></td>
                          <td v-else>{{row.sender_full_name}}</td>
                          <td v-html="row.sender_username"></td>
                          <td v-if="row.user_id != row.recepient"><inertia-link class="nav-link" :href="route('user_profile', row.recepient_slug)">{{row.recepient_full_name}}</inertia-link></td>
                          <td v-else>{{row.recepient_full_name}}</td>
                          <td v-html="row.recepient_username"></td>
  ...
  watch: {
    filter_rows_form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('funds_transfer_history'), pickBy(this.filter_rows_form), { preserveState: true,preserveScroll: true })
      }, 150),
    },
  },
  mounted() {
