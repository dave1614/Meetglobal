                  ...
                  <div class="table-div material-datatables table-responsive" style="" v-if="all_users.data.length > 0">
                    <table id="all-registered-users-table" class="table table-test table-striped table-bordered nowrap hover display" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr v-for="row in all_users.data" :key="row.index">
                          <td>{{row.index}}. </td>
                          ...
                          <td v-html="row.date_of_registration"></td>
                          <td><inertia-link class="nav-link" :href="route('user_profile', row.coop_db_sponsor_slug)">{{row.coop_db_sponsor_username}}</inertia-link></td>
                          <td><inertia-link class="nav-link" :href="route('user_profile', row.coop_db_placement_coop_slug)">{{row.coop_db_placement_coop_user_name}}</inertia-link></td>
                          <td v-html="row.positioning"></td>
  ...
  watch: {
    filter_rows_form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('cooperative_members_admin'), pickBy(this.filter_rows_form), { preserveState: true,preserveScroll: true })
      }, 150),
    },
  },
  mounted() {
