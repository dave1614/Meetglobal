          ...
          <div class="row justify-content-center" style="">
            <div class="card" id="main-card">
                ...
                <div class="card-body">
                    ...
                    <table id="all-registered-users-table" class="table table-test table-striped table-bordered nowrap hover display" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr v-for="row in all_history.data" :key="row.index">
                          ...
                          <td>{{row.amount_paid_str}}</td>
                          <td>{{row.balance_str}}</td>
                          <td>{{row.service_charge_str}}</td>
                          <td>{{row.profit_made_str}}</td>
                          <td>{{row.charged_num}}</td>
                          <td>{{row.total_amount_charge_str}}</td>
                          <td>{{row.date_time}}</td>
                  ...
                  <h4 style='font-weight: bold; margin-top: 30px;'>Total Amount Requested: <em class='text-primary'>{{total_amount_requested}}</em></h4>
                  <h4 style='font-weight: bold;'>Total Amount Paid Back: <em class='text-primary'>{{total_amount_paid_back}}</em></h4>
                  <h4 style='font-weight: bold;'>Total Balance: <em class='text-primary'>{{total_balance,2}}</em></h4>
  ...
  watch: {
    filter_rows_form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('view_advance_loan_history'), pickBy(this.filter_rows_form), { preserveState: true,preserveScroll: true })
      }, 150),
    },
  },
  mounted() {
