                      ...
                      <tbody>                  
                        ...
                        <tr v-for="row in all_history.data" :key="row.index">
                          ...
                          <td class="td-actions" >
                            ...
                            <button v-if="row.order_id_cut == 'PS' && (row.type == 'data' || row.type == 'electricity' || row.type == 'cable' || row.type == 'airtime_to_wallet' || row.type == 'bulk_sms')" type="button" title="Track This Order" class="btn btn-success" @click.stop="trackThisOrderPayscribe(row.order_id)">
                              Track
                            </button>

                            <button v-if="row.sub_type == 'payscribe_epin'" type="button" title="Track This Order" class="btn btn-success" @click.stop="trackThisOrderPayscribeEpin(row.order_id)">
                              Track
                            </button>
  ...
  watch: {
    filter_rows_form: {
      ...
      handler: throttle(function() {
        this.$inertia.get(this.route('users_vtu_history',this.users_user_id), pickBy(this.filter_rows_form), { preserveState: true,preserveScroll: true })
      }, 150),
    },
  },
  mounted() {
