            ...
            <div class="card" id="edit-user-profile-card">
                ...
                <div class="card-body">
                  ...
                  <form id="edit-user-profile-form" @submit.prevent="submitEditUsersProfileForm">  
                    ...
                    <text-input v-model="edit_user_profile_form.transaction_password" :error="edit_user_profile_form.errors.transaction_password" type="password" label="Enter Transaction Password" id="transaction_password" placeholder=""/>
...
export default {
  ...
  data() {
    return {
      ...
      edit_user_profile_form: this.$inertia.form({
        ...
        phone_number: this.user_info.phone,
        email_address: this.user_info.email,
        address: this.user_info.address,
        user_id: this.user_info.id,
        transaction_password: null,
      }),

      card_title: "Edit <em class='text-primary'>" + this.user_info.user_name + "'s</em> Profile",
  ...
  methods: {
    submitEditUsersProfileForm() {
      var self = this;
      self.edit_user_profile_form.post(self.route('process_edit_profile'), {
        preserveScroll: true,
        onSuccess: (page) => {
          ...
          }else if(response.incorrect_transaction_password){
            swal({
              title: 'Ooops',
              text: "The Transaction Password Entered Was Invalid. Please Enter The Valid One.",
              type: 'error'
            })
          }else{
